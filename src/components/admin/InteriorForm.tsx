...
interface InteriorFormProps {
  ...
  onSave: (option: InteriorOption) => void;
  onCancel: () => void;
}

export function InteriorForm({ option, onSave, onCancel }: InteriorFormProps) {
  const [formData, setFormData] = useState(option);

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
        <h3 className="text-lg font-medium mb-4">{option.id ? 'Edit' : 'Add'} Option</h3>
